...
PLUGINLIB_EXPORT_CLASS(occupancy_grid_generation_pkg::OccupancyGridGeneration, nodelet::Nodelet)
...
namespace occupancy_grid_generation_pkg
{
  ...
  void OccupancyGridGeneration::onInit()
  {
    ...
    occ_pub = private_nh.advertise<nav_msgs::OccupancyGrid>("occupancy", 1);
    ...
    // timer = private_nh.createTimer(ros::Duration(0.1), &OccupancyGridGeneration::timer_cb, this);
  }

  void OccupancyGridGeneration::timer_cb(const ros::TimerEvent&)
  {
    // If you have access to vehicle information, fill this in.
  ...
  void OccupancyGridGeneration::add_points_to_the_DST(pcl::PointCloud<DDLPointType>& cloud)
  {
    ...
    for (size_t i = 0; i < cloud.points.size(); i++)
    {
      ...
      int x = std::floor(cloud.points[i].x/res);
      int y = std::floor(cloud.points[i].y/res);
      ...
      if (x >= -64 && y>= -64 && x <= 63 && y <= 63) {
        ...
        if (slope > 0 && slope <= 1 && x>0){
          ...
          ray_tracing_vertical_fill(y);
        }
        else if (x == 0 && y < 0) {
          ray_tracing_vertical_n_fill(y);
        }
        else if (x > 0 && y == 0) {
          ray_tracing_horizontal_fill(x);
        }
        else if (x < 0 && y == 0) {
          ray_tracing_horizontal_n_fill(x);
        }
      }
    }
  }
  ...
  void OccupancyGridGeneration::ray_tracing_vertical_fill(int y2)
  {
    int x1 = 0;
    int y1 = 0;
    int x2 = 0;
    ...
    for (int y = y1; y <= y2; y++){
      ...
      if (meas_occ[x1+64][y+64] == meas_mass) {
        break;
      }
      meas_free[x1+64][y+64] = meas_mass;
    }
    meas_occ[x2+64][y2+64] = meas_mass;
    meas_free[x2+64][y2+64] = 0.0;
  }

  /// FIX
  void OccupancyGridGeneration::ray_tracing_vertical_n_fill(int y2)
  {
    int x1 = 0;
    int y1 = 0;
    int x2 = 0;
    //y1 = y1-1; //y1 = y1+1; // CHECK WHY?
  ...
  void OccupancyGridGeneration::ray_tracing_horizontal_fill(int x2)
  {
    int x1 = 0;
    int y1 = 0;
    int y2 = 0;
    ...
    for (int x = x1; x < x2; x++){
      if (meas_occ[x+64][y1+64] == meas_mass) {
        break;
      }
      meas_free[x+64][y2+64] = meas_mass;
    }
    meas_occ[x2+64][y2+64] = meas_mass;
    meas_free[x2+64][y2+64] = 0.0;
  }

  void OccupancyGridGeneration::ray_tracing_horizontal_n_fill(int x2)
  {
    int x1 = 0;
    int y2 = 0;
    int y1 = 0;
...
/// FIX
  ...
  void OccupancyGridGeneration::ray_tracing_approximation_y_increment(int x1, int y1, int x2, int y2, int flip_x, int flip_y, bool inclusive)
  {
    ...
    for (int x = x1, y = y1; x < x2; x++){
      ...
      slope_error += slope; // Here or below.
    }
    ...
    if (inclusive==false) {
      ...
      meas_free[flip_x*x2+64][flip_y*y2+64] = meas_mass;
    }
  }
  ...
  void OccupancyGridGeneration::ray_tracing_vertical_n(int y2)
  {
    int x1 = 0;
    int y1 = 0;
  ...
  void OccupancyGridGeneration::plotting()
  {
    ...
    occupancy_msg.header.frame_id = "velodyne"; //TODO: Make sure the frame is the correct one.
    ...
    for (unsigned int i = 0; i < grid_size; i++){
      for (unsigned int j = 0; j < grid_size; j++){
        occupancy_msg.data.push_back(prob_O_plot[j][i]); // CHANGE BACK
  ...
  void OccupancyGridGeneration::get_mass()
  {
    for (unsigned int i = 0; i < grid_size; i++){
      for (unsigned int j = 0; j < grid_size; j++){
        ...
        prob_O_plot[i][j] = 100*(0.5 * up_occ[i][j] + 0.5*(1.0 - up_free[i][j])); //meas_free[i][j] * 100;
      }
    }
  }
  ...
  void OccupancyGridGeneration::add_free_spaces_to_the_DST()
  {
    ...
    */
    ...
    ang = (i*0.1f);
    // std::cout <<"Ang: " << ang <<" "<<angles[(int)(ang)] <<  std::endl;
      if (angles[(int)(ang)] == false){
        int x,y;
        if (ang>0.0f && ang<45.0f){
          x = 63;
          y = std::floor(tan(ang*PI/180.0f)*x); // std::floor
        }
        else if (ang==45.0f){
          x = 63;
          y = 63;
          ...
          y = 63;
          x = std::floor(y/tan(ang*PI/180.0f));
        }
        else if (ang>90.0f && ang<=135.0f){
          y  = 63;
          x = std::floor(y/tan((ang-180.0f)*PI/180.0f));
        }
        else if (ang>135.0f && ang<180.0f){
          x = -64;
          y = std::floor(tan((ang-180.0)*PI/180.0f)*x);
        }
        else if (ang>180.0f && ang<=225.0f){
          x = -64;
          y = std::floor(tan((ang-180.0f)*PI/180.0f)*x);
        }
        else if (ang>225.0f && ang<270.0f){
          y  = -64;
          x = std::floor((y+1)/tan((ang-180.0f)*PI/180.0f));
        }
        else if  (ang>270.0f && ang<= 315.0f){
          y = -64;
          x = std::floor((y+1)/tan((ang-360.0f)*PI/180.0f));
        }
        else if (ang>315.0f && ang<360.0f){
          x = 63;
          y = std::floor(tan((ang-360.0f)*PI/180.0f)*x);
        }
        else if (ang == 0.0f || ang == 360.0f){
          ray_tracing_horizontal(63);
          continue;
        }
        else if (ang == 90.0f){
          ray_tracing_vertical(63);
          continue;
        }
        else if (ang == 180.0f){
        ...
        if (x >= -64 && y>= -64 && x <= 63 && y <= 63) {
          ...
          std::cout << "Error. Free spaces are out of scope:" << x << " " << y << std::endl;
        }
      }
