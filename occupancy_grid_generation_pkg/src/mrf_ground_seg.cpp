...
PLUGINLIB_EXPORT_CLASS(occupancy_grid_generation_pkg::MrfGroundSeg, nodelet::Nodelet)
...
namespace occupancy_grid_generation_pkg {
  ...
  void MrfGroundSeg::GroundSegMRFCallback(const DDLPointCloud::ConstPtr& originalPC) {
    // ROS_INFO("Points for the segmentation received.");
  	...
  	if ((abs(point.x) <= maxXYCoord) && (abs(point.y) <= maxXYCoord)) { // && (point.z <= 0.28) point.x>=0
